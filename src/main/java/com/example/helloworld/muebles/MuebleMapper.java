...
public interface MuebleMapper {
    @Select("SELECT * FROM Mueble") //Se ingresa a la tabla muebles
    public List<Mueble> getAll();
    
    @Select("SELECT * FROM Mueble WHERE id = #{id} ")  // Se selecciona el id desde la tabla mueble
    public Mueble getById(int id);

    @Insert("INSERT INTO Mueble VALUES(#{id},#{nombre},#{color},#{precio},#{agotado})")  //Se insertan los valores a la tabla mueble
